...
class web_scrape:
    ...
    def vol_scrape(self):
        ...
        for i in chapter_df.index:
            ...
            while first_page <= last_page:
                ...
                if self.vol == 0: prompt = f"Extracting  Chapter:{current_chapter:2.0f} Page: {page_counter}"
                else: prompt = f"Extracting  Volume: {self.vol} Chapter:{current_chapter:2.0f} Page: {page_counter}"

                self.loading_animation(prompt, page_counter)
                
                first_page += 1
                page_counter += 1
        self.loading_animation(prompt, 9)
        print('Done\n')
        self.archive()
    ...
    def loading_animation(self, prompt, page_counter):
        ...
        sys.stdout.write(f"\r{prompt}... " + animation[page_counter % len(animation)]) # For animation
        sys.stdout.flush() # For animation
